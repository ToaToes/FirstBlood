    ...
    void SpawnEnemy()
	{
		if (Enemy != null && actualWaveCount < waveCount)
		{
            for (int i = 0; i < SpawnPerCycle; i++)
            {
                ...
                Vector3 FinalSpawnPosition = transform.position + new Vector3(RandomRadius, 0.0f, RandomRadius2);
                ...
                GameObject EnemySpawned = Instantiate(Enemy, FinalSpawnPosition, rot) as GameObject;
                ...
                if (EnemyPatrolRoute && startInRandomWaypoint)
                {
                    ...
                    EnemySpawned.transform.position = EnemySpawned.GetComponent<PrEnemyAI>().waypoints[rndm].position;
                }
